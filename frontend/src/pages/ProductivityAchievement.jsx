...
const ProductivityAchievement = () => {
  ...
  const productivityTeamData = useMemo(() => {
    ...
    picData.forEach(row => {
      ...
      const role = row[picRoleCol] || '-';
      picRoles[picName] = role;
    ...
    const finalData = Object.values(groupedByPic).map(picObj => {
      const finalObj = { name: picObj.name, role: picRoles[picObj.name] || '-', Total: 0 };
                ...
                <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'stretch' }}>
                  ...
                  <div style={{ overflowX: 'auto', overflowY: 'auto', flex: 1, maxHeight: '480px', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '6px', WebkitOverflowScrolling: 'touch' }} className="custom-scrollbar">
                    <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0, color: 'white', fontSize: '0.75rem' }}>
                      <thead>
                        <tr>
                          <th style={{ position: 'sticky', top: 0, zIndex: 5, backgroundColor: '#0f172a', padding: '0.3rem 0.5rem', textAlign: 'left', borderRight: '1px solid rgba(255,255,255,0.1)', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>PIC</th>
                          <th style={{ position: 'sticky', top: 0, zIndex: 5, backgroundColor: '#0f172a', padding: '0.3rem 0.5rem', textAlign: 'center', width: '60px', borderRight: '1px solid rgba(255,255,255,0.1)', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>Role</th>
                        ...
                        {combinedTicketTeamData.data.map((row, idx) => {
                          ...
                          return (
                            <tr key={idx} style={{ backgroundColor: rowBg }}>
                              ...
                              <td style={{ padding: '0.3rem 0.5rem', textAlign: 'center', borderRight: '1px solid rgba(255,255,255,0.05)', borderBottom: '1px solid rgba(255,255,255,0.05)', whiteSpace: 'nowrap', width: '60px' }}>{row.role}</td>
                      ...
                      <tbody>
                        {combinedTicketTeamData.data.map((row, idx) => {
                          ...
                          return (
                            <tr key={idx} style={{ backgroundColor: rowBg }}>
                              ...
                              <td style={{ backgroundColor: rowBg, padding: '0.3rem 0.5rem', textAlign: 'center', borderRight: '1px solid rgba(255,255,255,0.05)', borderBottom: '1px solid rgba(255,255,255,0.05)', whiteSpace: 'nowrap', width: '60px' }}>{row.role}</td>
                              {combinedTicketTeamData.columns.map(col => (
                        ...
                        {combinedTicketTeamData.data.length > 0 && (
                          <tr style={{ backgroundColor: '#1e293b', fontWeight: 'bold' }}>
                            ...
                            <td style={{ position: 'sticky', bottom: 0, zIndex: 5, backgroundColor: '#1e293b', padding: '0.3rem 0.5rem', borderRight: '1px solid rgba(255,255,255,0.1)', borderTop: '1px solid rgba(255,255,255,0.1)' }}></td>
                            {combinedTicketTeamData.columns.map(col => {
